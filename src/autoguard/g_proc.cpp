...
static constexpr uint32_t DEFAULT_WEIGTH_PCT100    = 100;
...
public:
    Scheduler():
    	processes({}), smooth_scale_x100(100), last_recompute_ms(0),
    ...
    void tick()
    {
		...
		for (proc_size_t i = 0; i < processes.count(); i++) {
			...
			if (pr.exec_ewma_us_pct100 <= 0) {
				pr.exec_ewma_us_pct100 = new_us100;
			} else {
				pr.exec_ewma_us_pct100 = (
					((int64_t)(100 - EWMA_ALPHA_PCT100) * pr.exec_ewma_us_pct100 ) +
    ...
    void print_status()
    {
        ...
        for (proc_size_t i = 0; i < processes.count(); ++i) {
            ...
            int64_t exec_us100  = p.exec_ewma_us_pct100;
            uint32_t load_x100  = 0;
            if (periodC > 0) {
            	load_x100 = (uint32_t)(exec_us100 / (periodC * MILLIS_US));
            }

            int64_t load_scaled = p.last_load_scaled;
            sum_weighted_load += (int64_t)p.weight_pct100 * load_scaled;
			...
			gprint(
				...
				load_x100 / FIX, __abs(load_x100 % FIX),
				p.weight_pct100 / FIX, __abs(p.weight_pct100 % FIX),
				p.scale_raw_pct100 / FIX, __abs(p.scale_raw_pct100 % FIX),
				p.scale_smooth_pct100 / FIX, __abs(p.scale_smooth_pct100 % FIX),
				p.realtime ? "YES" : "NO"
			);
		}
    	print_div_line();
		...
		int64_t total_percent = U_weighted_scaled / 100;

        printPretty(
			"Total weighted load: %ld%%  |  Target utilization: %ld%%\n",
        ...
        for (proc_size_t i = 0; i < processes.count(); ++i) {
            Process<>& p = processes[i];
			int32_t load_s = calc_load_scaled(p.last_exec_us, p.current_delay_ms);
			int load_percent_times100 = (load_s * 100) / LOAD_SCALE;
			if (load_percent_times100 > 500) {
				printPretty(
...
static Scheduler<
	...
	TaskList<
		Process<_scheduler_load_show,         10 * SECOND_MS, true,  true, DEFAULT_WEIGTH_PCT100>,
		Process<_scheduler_recompute_scaling, RECOMPUTE_MS,   true,  true, DEFAULT_WEIGTH_PCT100>,
		Process<_scheduler_error_check,       SECOND_MS,      true,  true, DEFAULT_WEIGTH_PCT100>,
#ifndef GSYSTEM_NO_MEMORY_W
		Process<memory_watchdog_check,        100,            false, true, DEFAULT_WEIGTH_PCT100>,
#endif
#ifndef GSYSTEM_NO_SYS_TICK_W
		Process<sys_clock_watchdog_check,     SECOND_MS / 10, false, true, DEFAULT_WEIGTH_PCT100>,
#endif
#ifndef GSYSTEM_NO_RAM_W
		Process<ram_watchdog_check,           5 * SECOND_MS,  false, true, DEFAULT_WEIGTH_PCT100>,
#endif
#if !defined(GSYSTEM_NO_ADC_W)
		Process<adc_watchdog_check,           1,              true,  true, DEFAULT_WEIGTH_PCT100>,
#endif
#if defined(STM32F1) && !defined(GSYSTEM_NO_I2C_W)
		Process<i2c_watchdog_check,           5 * SECOND_MS,  false, true, DEFAULT_WEIGTH_PCT100>,
#endif
#ifndef GSYSTEM_NO_RTC_W
		Process<rtc_watchdog_check,           SECOND_MS,      false, true, DEFAULT_WEIGTH_PCT100>,
#endif
#if !defined(GSYSTEM_NO_POWER_W) && !defined(GSYSTEM_NO_ADC_W)
		Process<power_watchdog_check,         1,              true,  true, DEFAULT_WEIGTH_PCT100>,
#endif
#ifndef GSYSTEM_NO_DEVICE_SETTINGS
		Process<settings_update,              500,            false, true, DEFAULT_WEIGTH_PCT100>,
#endif
		Process<btn_watchdog_check,           5,              false, true, DEFAULT_WEIGTH_PCT100>
	>
> scheduler;
